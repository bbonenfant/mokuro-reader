...
extern "C" {
    ...
    async fn load(this: &Archive, file: web_sys::File) -> Result<JsValue, JsValue>;

    #[wasm_bindgen(catch, method)]
    async fn generate(this: &Archive) -> Result<JsValue, JsValue>;

    #[wasm_bindgen(catch, method)]
    async fn get(this: &Archive, name: &str) -> Result<JsValue, JsValue>;

    #[wasm_bindgen(catch, method)]
    async fn set(this: &Archive, name: &str, data: Vec<u8>) -> Result<JsValue, JsValue>;

    #[wasm_bindgen(method)]
    fn add_directory(this: &Archive, name: &str);
}
...
pub async fn extract_ziparchive(
    ...
    let mut volume = {
        let mut volume = {
            let data = js_to_u8_vec(archive.get(METADATA_FILE).await?);
    ...
    let cover_object_url = {
        let cover_data = js_to_u8_vec(archive.get(cover).await?);
    ...
    for (page_name, ocr_name) in volume.pages.iter() {
        let image_data = {
            let image_data = js_to_u8_vec(archive.get(page_name).await?);
            PageImage::new(page_name, &image_data[..])
        };

        let page_ocr = {
            let data = js_to_u8_vec(archive.get(ocr_name).await?);
...
pub async fn create_ziparchive(
    ...
    archive.set(METADATA_FILE, metadata).await?;
    archive.add_directory("_ocr/");
    ...
    for (page_name, ocr_name) in volume.pages.iter() {
        ...
        archive.set(page_name, image_data).await?;

        let ocr_data = serde_json::to_vec(&ocr)?;
        archive.set(ocr_name, ocr_data).await?;
    }

    let name = &format!("{}.mbz.zip", volume.title);
    let buffer = js_to_u8_vec(archive.generate().await?);
